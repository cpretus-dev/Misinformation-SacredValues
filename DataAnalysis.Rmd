...
#-------------------------------------------------
NumSacredFit <- ESdf %>% 
  lmer(sharing ~ value * NumOfSacredValues + (1|identity), .) 
...
CrtFit <- ESdf %>% 
  lmer(sharing ~ value * crt_correct + (1|identity), .) 
...
#               fMRI study
#-------------------------------------------------
# A similar effect was found in the fMRI sample (N = 36)
CrtFit <- MRdf %>% 
  lmer(sharing ~ value * crt_correct + (1|identity), .)
...
AffilFit <- ESdf %>% 
  lmer(sharing ~ votersGroup + (1|identity), .) 
...
FusedFit <- USdf %>% 
  lmer(sharing ~ FusedTrump + (1|identity), .) 
...
FusedFit <- USdf %>% 
  ...
  lmer(sharing ~ FusedRepublican + (1|identity), .) 
...
# Identity fusion with extreme political options moderated effect of sacred values.
Identity fusion with the far-right in Spain and Trump in the US was associated with *increased sharing* of posts including sacred values (vs. non-sacred values).
...
#               Spanish survey
#-------------------------------------------------
## identity fusion with the far-right increased sharing of posts with sacred values (vs. non-sacred values)

ValueFusedFit <- ESdf %>% 
  filter(votersGroup %in% "far-right") %>% 
  lmer(sharing ~ value * Fused + (1|identity), .) 

summary(ValueFusedFit)
PairwiseComparison <- emmeans(ValueFusedFit, pairwise ~ value | Fused)
PairwiseComparison

## however, no interaction with value was found with fusion with the center-right
ValueFusedFit <- ESdf %>% 
  filter(votersGroup %in% "center-right") %>% 
  lmer(sharing ~ value * Fused + (1|identity), .) 
...
ValueFusedFit <- USdf %>% 
  lmer(sharing ~ value * FusedTrump + (1|identity), .) 
...
ValueFusedFit <- USdf %>% 
  filter(FusedTrump %in% F) %>% 
  lmer(sharing ~ value * FusedRepublican + (1|identity), .) 
...
#               Spanish survey
#-------------------------------------------------
## factchecking had no effect on sharing when compared to control trials. However, there is an increase in sharing in fact-check trials compared to baseline. In the US survey we investigate whether this increase from baseline is a temporal effect or a backfire effect from fact-checking by including a control group with no fact-checks.
FactCheckFit <- ESdf %>% 
  lmer(sharing ~ factcheck + (1|identity), .)
...
FactCheckFit <- ESdf %>% 
  lmer(sharing ~ factcheck * FactCheckGroup + (1|identity), .)
...
FactCheckFit <- USdf %>% 
  ...
  lmer(sharing ~ Group * block + (1|identity), .)
...
FactCheckFit <- USdf %>% 
  ...
  lmer(sharing ~ factcheck * FusedTrump + (1|identity), .)
...
FactCheckFit <- ESdf %>% 
  lmer(sharing ~ factcheck * value + (1|identity), .)
...
## we thought language would affect non-sacred values to a greater extent compared to sacred values, because sacred values are highly salient regardless of the wording, but the interaction between language and value was non-significant (p = 0.07)
LanguageFit <- ESdf %>% 
  lmer(sharing ~ language * value + (1|identity), .)
...
LanguageFit <- USdf %>% 
  lmer(sharing ~ language * FusedTrump + (1|identity), .)
